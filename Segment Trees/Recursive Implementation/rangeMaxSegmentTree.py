...
''' Range Max-Segment Tree Implementation using Python List '''
...
class buildSegmentTree:
    ...
    def _updateRange(self, qs, qe, s, e, treeIndex, val):
        ...
        # Partial Overlap:
            ...
            self.tree[treeIndex] = max(self.tree[2*treeIndex], self.tree[2*treeIndex+1])

tree = buildSegmentTree([-1,1,-1,0,3]) # create a segment tree
print(tree.query(0,5)) # prints 3
tree.updateRange(0, 5, 6) # add 6 to each element from index 0 to 5.
print(tree.query(0,5)) # prints 9